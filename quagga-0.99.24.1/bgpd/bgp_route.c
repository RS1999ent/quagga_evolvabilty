...
static void bgp_extract_wiser_info(const struct integratedAdvert *advert, int *wiserCost, int *normalization)
{
	...
	if(advert != NULL){
		...
		{
			if(advert->pathAttributes->fields->protoID == WISER)
			{
				pathFields = advert->pathAttributes->fields;
			}
		}
		if(pathFields != NULL){
			/*grab the wiser cost and normalization */
			for(int i = 0; i < pathFields->numFields; i++)
			{
				struct field field = pathFields->fields[i];
				if(strcmp(field.fieldName, "WISERCOST") == 0)
				{
					*wiserCost = atoi(field.aFieldValue);
				}
				if(strcmp(field.fieldName, "NORMALIZATION") == 0)
				{
					*normalization = atoi(field.aFieldValue);
				}
			}
		}
	}
...
/* Compare two route entity using wiser decision process */
static int
bgp_wiser_info_cmp(struct bgp *bgp, struct bgp_info *new, struct bgp_info *exist, int *paths_eq, struct integratedAdvert *new_advert, struct integratedAdvert *exist_advert)
{
  ...
  /* WISER attributes */
  int new_wiserCost = -1, exist_wiserCost = -1;
  int new_normalization = -1, exist_normalization = -1;
  ...
  *paths_eq = 0;
  ...
  if (new_pref < exist_pref)
   ...
   /* 3 lowest WISER cost  */
  //extract wiserCost for new and existing advertisements
  bgp_extract_wiser_info(new_advert, &new_wiserCost, &new_normalization);
  bgp_extract_wiser_info(exist_advert, &exist_wiserCost, &exist_normalization);

  /*
   * what to do if one path has wiser info and another doesn't.  this is signified
   * by the values being -1. right not, only goes through step if both have wiser info
   */

  if(new_wiserCost != -1 && exist_wiserCost != -1){
	  if(new_wiserCost/new_normalization < exist_wiserCost/exist_normalization)
	  {
		  return 1;
	  }
	  else
		    ...
		    struct bgp_info_pair *result)
{
  ...
  for (ri = rn->info; (ri != NULL) && (nextri = ri->next, 1); ri = nextri)
  {
	  ...
	  int bgp_wiser_decision = bgp_wiser_info_cmp(bgp, ri, new_select, &paths_eq, new_advert, exist_advert);

	  if(bgp_wiser_decision) {
